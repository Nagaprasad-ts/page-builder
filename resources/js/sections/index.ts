 ...
 */
const modules = import.meta.glob<SectionRegistration>('./*.tsx', { eager: true });
...
export function getSectionsByCategory(): Record<string, SectionRegistration[]> {
    return Object.values(sectionRegistry).reduce<Record<string, SectionRegistration[]>>((acc, reg) => {
...
export function getDefaultProps(sectionType: string): Record<string, unknown> {
    ...
    return Object.entries(registration.schema).reduce<Record<string, unknown>>((acc, [key, field]) => {
