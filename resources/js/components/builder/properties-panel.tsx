...
export function PropertiesPanel({ section, onChange, onOpenMediaPicker }: Props) {
    if (!section) {
        return (
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="border-b border-border px-4 py-3">
                <h3 className="text-sm font-semibold">{meta.name.charAt(0).toUpperCase() + meta.name.slice(1)}</h3>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-4">
                {Object.entries(schema).map(([key, def]) => (
                    <FieldEditor
                        ...
                        onChange={(v) => {
                            onChange(section.id, { ...section.props, [key]: v });
                        }}
