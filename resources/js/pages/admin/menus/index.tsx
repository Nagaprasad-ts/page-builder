...
export default function MenusIndex({ menus }: Props) {
    const [deletingMenu, setDeletingMenu] = useState<MenuWithCount | null>(null);
    ...
    return (
        <>
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                            Manage navigation menus for desktop, mobile, and footer.
                        </p>
                    </div>
                    <Button asChild>
                    ...
                    <div className="overflow-hidden rounded-xl border border-border">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-border">
                                {menus.map((menu) => (
                                    <tr key={menu.id} className="group bg-background hover:bg-muted/30">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                                                <Button asChild size="sm" variant="ghost">
                                                    <Link href={`/admin/menus/${menu.id}/edit`}>Edit</Link>
                                                </Button>
                                                <Button
                                                    size="sm"
                                                    variant="ghost"
            ...
            <Dialog open={!!deletingMenu} onOpenChange={(v) => !v && setDeletingMenu(null)}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Delete menu?</DialogTitle>
                        <DialogDescription>
                            "{deletingMenu?.name}" and all its items will be permanently deleted.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
